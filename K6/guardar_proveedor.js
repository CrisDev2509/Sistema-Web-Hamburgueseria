...
function generarRUC(A, B) {
  return Math.floor(Math.random() * (B - A + 1)) + A;
}

export default function () {
  const url = 'http://localhost:5139/Supplier/Create';
  const payload = 'Name=PROVEEDOR K6&RUC=' + generarRUC(10111111111, 10999999999) + '&Email=proveedor%40jmeter.com';
    const params = {
        headers: {
...
export function handleSummary(data) {
  return {
    "reports/agregar_proveedor.html": htmlReport(data),
  };
}